...
private ["_flatPos","_position"];
	
	_flatPos = [0,0,0];
	while {_flatPos distance2D [0,0,0] < 200} do {
		_flatPos = [getMarkerPos _dropZone, 10, 800, 5, 0, 0.3, 0, [[BaseArray, 800]], [0,0,0]] call BIS_fnc_findSafePos;
	};
	_position = _flatPos;
...
switch(_faction) do{
	case "paraMil":{
		...
		for "_i" from 1 to _squadSize do {
			...
			_GRP1 createUnit [_unit, getMarkerPos _startPos, [], 0, "NONE"];
		};
		sleep 3;
