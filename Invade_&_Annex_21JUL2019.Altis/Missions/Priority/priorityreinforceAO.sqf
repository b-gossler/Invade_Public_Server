...
Last modified: 16/5/18
modified: reworked to contain less spaghetti code
...
while { !_accepted } do {
    _flatPos = [getMarkerPos currentAO, missionsMinimumSpawnDistance, 5000, 5, 0, 0.2, 0, [], [0,0,0]] call BIS_fnc_findSafePos;
...
private _casHeliCode = {
    ...
    if (PrioHeliCount < 4) then {
        ...
        [_veh, false, getMarkerPos currentAO, (PARAMS_AOSize * (0.5 + (random 0.5)))] spawn AW_fnc_enemyAirEngagement;
        _enemiesArray = _enemiesArray + (units _reinforceGroup) + [_veh];
        mainAOUnits = mainAOUnits + (units _reinforceGroup) + [_veh];
        {_x addCuratorEditableObjects [units _reinforceGroup + [_veh], false];} forEach allCurators;
